...
export default function EnhancedEventForm({
  ...
  const provinces = [
    { value: "Hà Nội", label: "Tỉnh Tuyên Quang" },
    { value: "Hồ Chí Minh", label: "Hồ Chí Minh" },
    { value: "Đà Nẵng", label: "Đà Nẵng" }
  ];

  const districts = [
    { value: "Cầu Giấy", label: "Huyện Lâm Bình" },
    { value: "Hoàn Kiếm", label: "Quận Hoàn Kiếm" },
    { value: "Ba Đình", label: "Quận Ba Đình" }
  ];

  const wards = [
    { value: "Dịch Vọng", label: "Xã Phúc Yên" },
    { value: "Cầu Giấy", label: "Phường Cầu Giấy" },
    { value: "Dịch Vọng Hậu", label: "Phường Dịch Vọng Hậu" }
  ];
  ...
  const validateField = (fieldName: string, value: any): string | undefined => {
    switch (fieldName) {
      ...
      case 'eventName':
        return !value || value.trim() === '' ? 'Vui lòng nhập tên sự kiện' : undefined;
      case 'venueName':
        return !value || value.trim() === '' ? 'Vui lòng nhập tên địa điểm' : undefined;
      case 'province':
        return !value || value === '' ? 'Vui lòng chọn tỉnh/thành' : undefined;
      case 'district':
        return !value || value === '' ? 'Vui lòng chọn quận/huyện' : undefined;
      case 'ward':
        return !value || value === '' ? 'Vui lòng chọn phường/xã' : undefined;
      case 'street':
        return !value || value.trim() === '' ? 'Vui lòng nhập số nhà, đường' : undefined;
      case 'categoryId':
        return !value || value === 0 ? 'Vui lòng chọn thể loại sự kiện' : undefined;
      case 'description':
        return !value || value.trim() === '' ? 'Vui lòng nhập thông tin sự kiện' : undefined;
      case 'eventType':
        return !value || value === '' ? 'Vui lòng chọn loại sự kiện' : undefined;
      case 'orgThumbnail':
        return !orgThumbnail ? 'Vui lòng upload logo ban tổ chức' : undefined;
      case 'orgName':
        return !value || value.trim() === '' ? 'Vui lòng nhập tên ban tổ chức' : undefined;
      case 'orgDescription':
        return !value || value.trim() === '' ? 'Vui lòng nhập thông tin ban tổ chức' : undefined;
      default:
        return undefined;
    }
  };
  ...
  const handleFieldChange = (fieldName: string, value: any, setter: SetState<any>) => {
    setter(value);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="relative">
              <select
                value={ward}
                onChange={(e) => handleFieldChange('ward', e.target.value, setWard)}
