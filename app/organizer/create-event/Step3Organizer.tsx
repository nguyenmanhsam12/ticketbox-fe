...
interface Step3OrganizerProps {
  ...
  // New: dùng để render link preview
  eventName: string;
...
export default function Step3Organizer({
  ...
  // Nếu chưa có slug, mặc định sinh từ tên sự kiện (Step 1)
  const [hasEditedSlug, setHasEditedSlug] = useState(false);

  // Suy luận trạng thái "đã chỉnh tay" khi vào Step 3
  useEffect(() => {
    const initialSlug = slugify(eventName || "");
    if (link && link !== initialSlug) {
  ...
  // Khi tên sự kiện đổi ở Step 1 và quay lại Step 3, nếu chưa chỉnh tay thì cập nhật slug theo tên
  useEffect(() => {
    if (!hasEditedSlug && eventName) {
      const newSlug = slugify(eventName);
      if (link !== newSlug) {
        setLink(newSlug);
      }
    }
  }, [eventName, hasEditedSlug, link, setLink]);

  // URL đầy đủ luôn phụ thuộc vào slug hiện tại
  const computedUrl = `https://ticketbox.vn/${link ? `${link}-${eventId}` : ''}`;
  const fullUrl = (initialUrl && !hasEditedSlug) ? initialUrl : computedUrl;

  return (
    <div className="bg-gray-800 rounded-lg p-6 space-y-6">
      {/* Link sự kiện (slug) + Đường dẫn đầy đủ */}
      <div>
        ...
        <input
          type="text"
          value={link}
          onChange={(e) => { setHasEditedSlug(true); setLink(slugify(e.target.value)); }}
        ...
        {/* Full URL readonly, auto-updates */}
