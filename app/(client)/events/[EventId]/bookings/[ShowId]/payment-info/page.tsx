...
import isFreeImage from '@/src/image/isFree.png';
...
export default function PaymentInfo() {
  ...
  const [isFreeMethod, setIsFreeMethod] = useState('');
  ...
  const handlePayment = async () => {
    if (showData?.is_free) {
      // Sự kiện miễn phí → cần chọn phương thức Free
      if (!isFreeMethod) {
        toast.error('Vui lòng chọn phương thức miễn phí!');
        return;
      }
    } else {
      // Sự kiện trả phí → cần chọn cổng thanh toán
      if (!selectedMethod) {
    ...
    try {
      const res = await createOrderApi({
        bookingCode: cartData?.booking_code,
        showId: ShowId,
      ...
      router.push(
        `/events/${EventId}/bookings/${ShowId}/thankyou-${res?.data?.order.orderCode}`
      );
    } catch (error) {
                      ...
                      {showData?.is_free ? (
                        <div className="flex items-center gap-3">
                          <input
                            ...
                            onClick={() => setIsFreeMethod('freeMethod')}
                          />
                          <Image
                            src={isFreeImage}
                            alt="isFree"
